...
class QuestionsDisplayController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $multipleChoiceQuestions = MultipleChoiceQuestion::all();
            ...
            // Define the query for Long Questions
        $longQuery = LongQuestion::query();
        
        // Define the query for True/False Questions
        $trueFalseQuery = TrueFalseQuestion::query();
    ...
    if ($request->ajax()) {
        ...
        return view('admin.DisplayQuestions.filtered', compact('longQuestions','trueFalseQuestions','multipleChoiceQuestions'));
         ...
         // Retrieve paginated results for Long Questions
    $longQuestions = $longQuery->paginate(10); // 10 questions per page

    // Retrieve paginated results for True/False Questions
    $trueFalseQuestions = $trueFalseQuery->paginate(10);

    // Retrieve paginated results for Multiple Choice Questions
    $multipleChoiceQuestions = $multipleChoiceQuery->paginate(10);
